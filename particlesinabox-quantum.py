# vpython simulation - particles in a box - quantum collisions
#NOTES:
...
def particle_collision(particle1,particle2,dt):
    if ((particle1.pos-particle2.pos).mag < particle1.radius + particle2.radius):
        ...
        #Rotate the velocities a fixed amount - this corresponds with the 
        #probabilistic nature of collisions in quantum mechanics
        particle1.velocity = particle1.velocity.rotate(angle,axis=vector(rand_x,rand_y,rand_z))
        particle2.velocity = particle2.velocity.rotate(angle,axis=vector(rand_x,rand_y,rand_z))
        increment = dt/10
        #This while loop is the reason why it looks like the balls are teleporting
        while (particle1.pos-particle2.pos).mag < particle1.radius + particle2.radius:
            particle1.pos = particle1.pos + particle1.velocity * increment
            particle2.pos = particle2.pos + particle2.velocity * increment
            boundary_collision(particle1,particle_radius,box)
            boundary_collision(particle2,particle_radius,box)
